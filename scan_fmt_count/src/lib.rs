...
//!
//! ```rust
//! # #[macro_use] extern crate scan_fmt_count;
//! # fn main() {
//!   let (a,b,c, count) = scan_fmt_count!( "hello 12 345 bye", // input string
//!                            "hello {} {d} {}",  // format
...
//!   let (a,b,c, count) = scan_fmt_count!( "hello 12 345 bye", // input string
//!                            "hello {} {d} {}",  // format
//!                            u8, i32, i32);   // type of a-c Options
//!   assert_eq!( a.unwrap(), 12 ) ;
//!   assert_eq!( b.unwrap(), 345 ) ;
...
//! # #[macro_use] extern crate scan_fmt_count;
//! # fn main() {
//!   let (a,count) = scan_fmt_count!( "hello 12 345 bye", // input string
//!                            "hello {}",  // format
//!                            u8);   // type of a-c Options
//!   assert_eq!( a.unwrap(), 12 ) ;
//!   assert_eq!( count, 1 );
//! # }
//! ```
    ...
    ( $instr:expr, $fmt:expr, $arg1:ty, $($arg2:ty),* ) => {
        {
            ...
            ( match res.next() {
                Some(item) => { 
                    let item_res = item.parse::<$arg1>().ok();
                    match item_res {
              ...
              $( , match res.next() {
                    Some(item) => {
                        let item_res = item.parse::<$arg2>().ok();
                        match item_res {
                            Some(_) => {count = count + 1;},
                            None => {}
                        }   
...
#[test]
fn test_count_let_match() {
     if let (a,b,c, 3) = scan_fmt_count!( "hello 12 345 bye", // input string
                                           "hello {} {d} {}",  // format
                                            u8, i32, String)   // type of a-c Options
     {
         assert_eq!( a.unwrap(), 12 ) ;
     ...
     if let (_,_,_, 2) = scan_fmt_count!( "hello 12 345 bye", // input string
                                           "hello {} {d} {}",  // format
                                            u8, i32, String)   // type of a-c Options
     {
         assert!(false);
