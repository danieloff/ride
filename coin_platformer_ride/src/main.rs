...
impl GameState {
  ...
  const coin_count: usize = 45;

  const coin_positions: [[u32; 2]; GameState::coin_count] = [
    [16, 23],
    [33, 28],
    [41, 22],
    [20, 19],
...
fn reset_game() {
  ...
  let game_state = entity_get::<GameState>("game_state").unwrap();
  let mut game_state = game_state.borrow_mut();

  /* Set the starting level to demo.level */
//  game_state.current_level = asset_get("./levels/demo.level");
  game_state.level_score = 0;
  game_state.level_time = 0.0;
  
  /* New main character entity */
  let main_char = entity_get::<Character>("main_char").unwrap();
  let mut main_char = main_char.borrow_mut();
  main_char.position = Vec2::new(20.0, 20.0).mul(TILE_SIZE as f32); // vec2_new(20, 20), TILE_SIZE);
  main_char.velocity = Vec2::zero();
...
//  asset_handler("level", level_load_file);
  
  /* Load Assets */
  folder_load("./tiles/");
  folder_load("./backgrounds/");
  folder_load("./sounds/");
  folder_load("./levels/");
  
  /* Register some handlers for creating and destroying entity types */
  entity_handler(Character::new_rc);
  entity_handler(Coin::new_rc);
  entity_handler(GameState::new_rc);
  
  /* Create our main character */
  let _main_char = entity_new::<Character>("main_char", "character_type");
  //create our game state
  let _game_state = entity_new::<GameState>("game_state", "game_state_type");
 /* 
...
fn graphics_init(title: &str, width: u32, height: u32) -> WindowStuff {
...
fn main() {
  ...
  let WindowStuff {
    ...
    mut glfw,
    events,
  } = graphics_init("Birdy! Coin Platformer", 800, 600);
  ...
  while !window.should_close() {
    ...
    for (_, event) in glfw::flush_messages(&events) {
      match event {
        ...
        //  deltamouse = startmouse.sub(&Vec2::new(x as f32, y as f32).coords);
        //  println!("moved mouse");
        //}
        _ => {}
      }
