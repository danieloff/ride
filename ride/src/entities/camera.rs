...
use crate::engine::{Vec3, Mat4, mat4_view_look_at, mat4_perspective};
use cgmath::*;
/*
/**
*** :: Camera ::
...
pub fn camera_new() -> Rc<RefCell<Camera>> {
...
//pub fn camera_delete(_: Rc<RefCell<Camera>>) {}
...
pub fn camera_direction(c: &Camera) -> Vec3 {
  let mut res = c.target.sub(c.position);
  res = res.normalize();
  return res;
}
...
pub fn camera_view_matrix(c: &Camera) -> Mat4 {
  return mat4_view_look_at(c.position, c.target, Vec3::new(0.0, 1.0, 0.0));
}
...
pub fn camera_proj_matrix(c: &Camera, ratio: f32) -> Mat4 {
  return mat4_perspective(c.fov, c.near_clip, c.far_clip, ratio);
}
